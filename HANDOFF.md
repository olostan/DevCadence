...
| WP-M3B-5 | implemented, NOT accepted — round 1 (6 findings) and round 2 (4 findings) both fixed, awaiting round-3 re-review (EWP §14, §15) | current branch HEAD (see "Expected remote HEAD" above) | `go build ./...`, `go vet ./...`, `go test -count=1 ./...`, `go test -count=1 ./internal/schema/... ./internal/protocol/... ./internal/setup/... ./tests/...` all PASS | round 1 — [comment](https://github.com/olostan/DevCadence/pull/10#issuecomment-5826591074), 6 findings (static profiles still authoritative in Doctor/Planner, too-lossy ResourceInventory, BuildResourceInventory re-probing live state, Go/schema parity gaps, auth-blind viability check, reauth postcondition satisfiable by mere health) — all fixed, EWP §14. round 2 — [comment](https://github.com/olostan/DevCadence/pull/10#issuecomment-5827367817), 4 findings (MachineProfileRef not durably retrievable, auth-aware viability not propagated into discovery diagnostics + locality auth bypass, remaining Go/schema/report parity holes, `endpoint_authenticated` not operationally wired) — all fixed, EWP §15, not yet re-reviewed |
