...
## 18. Independent review round 5 disposition: 2 findings, all fixed

A fourth follow-up review ([PR #10 comment](https://github.com/olostan/DevCadence/pull/10#issuecomment-5830908798), owner, 2026-09-25, at head `b218af5`) confirmed round 4's fixes were substantially correct, then found two narrower integration/integrity gaps:

1. **`EndpointCredentialRefs` could still name a `CredentialRef` that is not actually configured.** Round 4 made the binding explicit, but not referentially valid: `NewDoctor` stored `opts.EndpointCredentialRefs` without checking each value resolves against `opts.CredentialRefs`, and an adapter-supplied `CognitionEndpoint.CredentialRef` was trusted as a WP4 RefID without the same check — so a typo'd binding (or an adapter declaring a ref nobody configured) still produced a syntactically valid but permanently unresolvable `endpoint_authenticated` condition. Fixed:
   - `NewDoctor` now structurally validates every `opts.CredentialRefs` entry and builds a `RefID` index; every `opts.EndpointCredentialRefs` value must resolve against that index or `NewDoctor` itself returns a configuration error — the binding fails at construction time, not silently downstream.
   - `discoverEndpoints` now only carries an adapter-declared `ep.CredentialRef` through as machine-verifiable when it resolves against the same index; an unrecognized adapter-supplied value is left out of the summary (diagnostic-only) rather than treated as a real binding.
   - `EndpointOperand.CredentialRefID` (in `Condition.Validate()`) and `setup-plan.schema.json`'s `credential_ref_id` now both apply the full bounded opaque-identifier / never-secret-looking contract `CredentialRef.RefID` itself uses — the schema reuses the canonical `noSecretLike` `$def` via cross-file `$ref` (the same mechanism as `credential-ref.schema.json`) rather than duplicating the secret-pattern list a third time.
   - `ResourceInventory.Validate()` now enforces (when `Credentials` is populated, the authoritative binding substrate) that every non-empty endpoint `CredentialRef` resolves to a `Credentials` entry, so the durable inventory can never internally contradict itself about which endpoints are bound to which credentials.
   - New tests: `TestNewDoctorRejectsEndpointCredentialRefBindingToUnknownRefID`, `TestDoctorDiscoverEndpoints_IgnoresUnrecognizedAdapterCredentialRef`, `TestSetupPlanEndpointAuthenticatedCredentialRefIDSchemaParity` (7 cases: valid/empty/secret-shaped/`token=`/invalid-character/at-limit/over-limit, each checked for Go↔schema agreement), `TestResourceInventoryValidation_EndpointCredentialRefMustResolve` (matches/unrelated-RefID/empty-Credentials-doesn't-trigger). `TestBuildResourceInventoryMalformedRefFailsClosed` was renamed `TestNewDoctorMalformedRefFailsClosed` and now asserts the failure at `NewDoctor` (earlier and stricter than the prior assertion point).
2. **The immutable profile archive could still lose immutability/readability through the read path and incomplete envelope validation.**
   - **2a.** `ReadProfileByID` deleted the archive file as a side effect of finding it corrupt (unparseable, invalid schema version, invalid profile, or ID-mismatched) — appropriate for an ordinary disposable cache, wrong for an immutable provenance archive: deleting on read let a later `archiveProfile` call see a missing path and freely create different bytes under the same `ProfileID`. Fixed: every one of those `os.Remove(path)` calls is gone; a corrupt archive entry now fails closed (not-found) and is left untouched.
   - **2b.** `archiveProfile`'s existing-entry check validated `existingEnv.Data` but not the envelope wrapping it — an existing file with a matching `Data` payload under an invalid envelope (bad `schema_version`, or an envelope `MachineFingerprint` disagreeing with `Data.MachineFingerprint`) would report idempotent success even though `ReadProfileByID`/`ReadProfileByRef` would immediately reject that same file — a write-succeeds/read-fails contradiction. Fixed: `archiveProfile` now validates the whole existing envelope (`SchemaVersion`, fingerprint coherence) before it may declare a match idempotent; any failure there is now a `CategoryConflict`, never a silent pass-through.
   - New tests: `TestReadProfileByID_NonMutatingOnCorruption` (malformed JSON / invalid schema version / invalid profile content, each asserting the file's bytes are byte-for-byte unchanged after the failed read), `TestArchiveProfile_InvalidExistingEnvelopeFailsClosed` (invalid envelope schema version; envelope/Data fingerprint disagreement), `TestArchiveProfile_SuccessAlwaysMakesReadProfileByRefResolve` (the round-5-required invariant test: both a fresh write and a subsequent idempotent no-op leave `ReadProfileByRef` resolvable for the same ref).

**Verification (this revision):** `go build ./...`, `go vet ./...`, `go test -count=1 ./...` (all 29 packages `ok`), `go test -race ./internal/setup/... ./internal/protocol/... ./internal/credentials/... ./internal/schema/... ./tests/...` (clean), `GOOS=windows GOARCH=amd64`/`GOOS=linux GOARCH=amd64 go build ./...` (clean), `git diff --check` (clean).

**Disposition:** both round-5 findings fixed; awaiting a round-6 review before WP-M3B-5 can be marked `accepted`.

## 19. Independent review round 6 disposition: 1 finding, fixed

A fifth follow-up review ([PR #10 comment](https://github.com/olostan/DevCadence/pull/10#issuecomment-5831801920), owner, 2026-09-25, at head `049768d`) confirmed round 5's two fixes were correct and found one narrow remaining referential-integrity gap:

**Duplicate configured `CredentialRef.RefID` values were still accepted, making the endpoint binding ambiguous.** The round-5 binding model assumes `CredentialRef.RefID -> exactly one configured CredentialRef`, but `NewDoctor` built its RefID index with `refIndex[ref.RefID] = true` (a bool set, indifferent to duplicates) and `NewCredentialsEndpointAuthChecker` separately built `refsByID[ref.RefID] = ref` (a map, silently "last write wins" on a duplicate) — so two `CredentialRefs` entries sharing a RefID (identical or different content) were accepted, and which one an `EndpointCredentialRefs` binding actually meant depended on map iteration order. The reviewer also noted `ResourceInventory`'s own duplicate-credential check only fires when `CredentialManager` is configured (`ObserveCredentials` returns no entries without one), so the no-manager Doctor path had no safety net at all.

Fixed with one shared helper rather than two independently-drifting checks: new `buildCredentialRefIndex(refs []protocol.CredentialRef) (map[string]protocol.CredentialRef, error)` in `internal/setup/conditions.go` validates every ref structurally and rejects a duplicate RefID outright (identical or different content — RefID is an identity, never a multimap key). Both `NewDoctor` and `NewCredentialsEndpointAuthChecker` now call this single helper instead of building their own index. `NewCredentialsEndpointAuthChecker`'s signature changed to `(*CredentialsEndpointAuthChecker, error)` so it can no longer silently construct an ambiguous checker; `NewExecutor` (its only production call site) propagates that error.

New tests: `TestNewDoctorRejectsDuplicateCredentialRefID` (table-driven: identical-content and different-content duplicate RefIDs, each run both with and without a `CredentialManager` configured — covering exactly the no-manager gap the review flagged), `TestNewCredentialsEndpointAuthChecker_RejectsDuplicateCredentialRefID`.

**Verification:** `go build ./...`, `go vet ./...`, `go test -count=1 ./...` (all 29 packages `ok`), `go test -race ./internal/setup/... ./internal/protocol/... ./internal/credentials/... ./internal/schema/... ./tests/...` (clean), `GOOS=windows GOARCH=amd64`/`GOOS=linux GOARCH=amd64 go build ./...` (clean), `git diff --check` (clean).

**Disposition:** the round-6 finding is fixed. The reviewer characterized this as "the only substantive blocker" remaining and said the next review should be the acceptance review — awaiting that round-7 review before WP-M3B-5 can be marked `accepted`.
