...
## 20. Sixth independent review: 1 security/protocol finding + canonical-doc sync, both fixed

A sixth review ([PR #10 comment](https://github.com/olostan/DevCadence/pull/10#issuecomment-5808795614), owner) confirmed both §19 integration fixes correct and explicitly closed every prior executor/MLX finding as not reopened, then found 2 remaining issues, both fixed in this revision — the reviewer stated these are the final blockers before WP-M3B-3 can be marked accepted:

1. **`ExecutableVerifiedOperand.VersionArgs []string` reopened an arbitrary-argv execution surface inside a supposedly closed, read-only condition.** §19's fix for the `hf version` vs `--version` mismatch added a free-form `[]string` argv field with no Go or JSON Schema constraint — architecturally different from "support `hf version`," since it let any plan (generated or malicious) turn `executable_verified` into a general argv carrier for whatever absolute path the plan names, defeating ADR-0014's closed-typed-protocol property (the PlanDigest binds approval to a *value*, not to an audit of arbitrary command semantics). Fixed by closing it: `VersionArgs []string` → `VersionProbe VersionProbeKind`, a new closed enum (`internal/protocol/setup.go`) with exactly two values, `double_dash_version` (default) and `version_subcommand`. The actual argv for each is decided in exactly one place, `internal/setup/conditions.go`'s `versionProbeArgs(kind)` — the executor, never the plan — and `Condition.Validate()` rejects any `VersionProbe` outside the enum via `VersionProbeKind.Valid()`. `schemas/setup-plan.schema.json`'s `version_args` array field became a `version_probe` string `enum`. `internal/setup/planner.go`'s `localModelRecipe.versionCheckArgs []string` → `versionProbeKind protocol.VersionProbeKind`, with the MLX recipe setting `VersionProbeVersionSubcommand`. New regressions, both requested explicitly: `TestConditionExecutableVerifiedRejectsArbitraryVersionProbe` (Go-level: an arbitrary string like `"-c; rm -rf /"` is rejected; both enum values plus the empty default validate) and `fixtures/protocol/setup-plan.invalid-version-probe.json` (JSON-Schema-level: wired into `tests/schema_fixtures_test.go`'s existing `.invalid-` fixture auto-discovery). Ollama's default probe and MLX's `version_subcommand` probe continue to pass their existing tests unchanged.
2. **Canonical `docs/adr/0014-guided-bootstrap-and-remediation.md` and `docs/WORK_PACKAGES.md` still described the pre-amendment, Ollama-specific protocol** (`ollama_pull_model`, `model_digest_present`, "resolved model digests") after WP-M3B-1's accepted runtime-agnostic amendment (§13) — leaving two normative sources in contradiction for the next agent. Fixed: ADR-0014 §1 now names `ensure_local_model`/`model_present` and describes the adapter-registry dispatch and the closed `VersionProbeKind` mechanism (finding 1, same revision); §7's supply-chain wording is now runtime-neutral (declared sources, immutable identities/revisions, per-adapter size verification, `LicenseReference` explicitly named as unverified provenance metadata); the "Costs" section's "resolved model digests" line now says "runtime-neutral immutable model identity/revision." `docs/WORK_PACKAGES.md`'s WP-M3B-1 deliverable list now names `ensure_local_model`/`model_present` with an explicit note that this is the accepted amendment and why (owner direction + DCI-055), rather than silently rewriting history.

**Verification (this revision):** `go build ./...`, `go vet ./...`, `gofmt -l internal/setup/*.go internal/protocol/setup*.go internal/environment/*.go internal/cognition/mlx/*.go`, `go test -count=1 ./...` (all packages, including the new invalid-fixture case), `go test -race ./internal/setup/... ./internal/cognition/mlx/... ./internal/environment/...`, `GOOS=windows GOARCH=amd64 go build ./...` — all clean.

**Disposition:** both findings fixed. Per the sixth reviewer: "Once those are fixed with the negative protocol/schema regression, I expect to mark WP-M3B-3 accepted / proceed to WP-M3B-4" — awaiting that confirmation.

## 21. Seventh independent review: WP-M3B-3 accepted

A seventh review ([PR #10 comment](https://github.com/olostan/DevCadence/pull/10#issuecomment-5809019161), owner, 2026-09-24) re-verified both §20 findings against the actual code, schema, tests and canonical docs, confirmed both resolved, did a final regression sweep over every previously closed finding across all six prior review rounds, and found no remaining substantive blocker.

**Disposition: WP-M3B-3 is accepted.** Per the reviewer: "GREEN to proceed to WP-M3B-4 (or another allowed next WP per the documented dependency chain)." PR #10 itself remains draft/open — it is the whole-milestone tracking PR, and WP-M3B-4 through WP-M3B-9 are not yet started.

This closes out a seven-round independent review cycle spanning the original executor implementation (§15, 10 findings), the architecture correction to a runtime-agnostic `LocalModelRuntimeAdapter` design (§16, owner-directed amendment to WP-M3B-1), and four further rounds hardening that redesign's own implementation quality, integration correctness, and protocol security (§17–§20, 8 + 3 + 2 + 2 findings respectively — 25 total findings across the whole WP-M3B-3 review history, all resolved). The accepted checkpoint's properties, per the reviewer's own summary, include: serialized machine-global execution and crash-safe ledger recovery; immutable PlanDigest approval and precondition recheck; no exported mutation bypass; bounded subprocess/output behavior; the runtime-neutral `ensure_local_model`/`model_present` protocol with MLX-LM and Ollama as true peer adapters; immutable MLX/Hugging Face revision pinning; source/size verification and recovery-completeness checks; exact verified mutation executable identity; real default-inventory `hf` CLI discovery; shared Hugging Face cache semantics between M3B setup and M3A cognition discovery; adapter-owned Ollama configuration; structural operation/postcondition identity binding; closed version-probe execution semantics; and coherent canonical ADR/scope-card documentation.
