...
The `cli_auth_call` guarantee is enforced structurally, not merely by adapter configuration: `internal/credentials.BoundedCLIAuthAdapter.ProbeAuth` inspects its own configured probe argv at evaluation time and reclassifies a version/help-shaped invocation (`--version`, `-v`, `--help`, …) to `probe_kind: cli_version_only` regardless of exit code, so a caller cannot promote a mere version check to `authenticated` by mislabeling it as an auth call.
